...
from src.cruds.chat.chat_crud import messagecrud
...
class CompanyService:
    ...
    def _get_applicant_email_from_model(self, applicant: Applicant | None) -> str | None:
        if not applicant:
            return None

        for attr in ("email", "applicant_email", "user_email"):
            value = getattr(applicant, attr, None)
            if value:
                return str(value)

        user = getattr(applicant, "user", None)
    ...
    async def _get_applicant_email_map(
        self,
        db: AsyncSession,
        ...
        unique_ids = [
            applicant_id
            for applicant_id in dict.fromkeys(applicant_ids)
            if applicant_id
        ]
        ...
        result = await db.execute(
            select(User.applicant_id, User.email)
            .where(
                User.applicant_id.in_(unique_ids),
        ...
        return {
            applicant_id: email
            for applicant_id, email in result.all()
            if applicant_id and email
        }
    ...
    async def delete_vacancy(
        ...
        try:
            vacancy = await self.get_company_vacancy_or_404(
                db=db,
            ...
            result = await db.execute(
                select(Status.id).where(Status.name == "Удалена")
            )
            deleted_status_id = result.scalar_one_or_none()

            if not deleted_status_id:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail='Статус "Удалена" не найден в справочнике.',
                )

            vacancy.status_id = deleted_status_id
            vacancy.updated_at = datetime.utcnow()
    ...
    def _is_hidden_vacancy_status_name(self, status_name: str | None) -> bool:
        value = str(status_name or "").lower().strip()
    ...
    def _is_hidden_vacancy(self, vacancy: Vacancy | None) -> bool:
        if not vacancy or not getattr(vacancy, "status", None):
            return False

        return self._is_hidden_vacancy_status_name(getattr(vacancy.status, "name", None))

    def _visible_vacancy_status_filter(self):
        hidden_status = or_(
    ...
    def map_employer_application(
        ...
        return {
            ...
            "vacancy": {
                ...
                "salary_max": vacancy.salary_max if vacancy else None,
                "currency": vacancy.currency.name if vacancy and vacancy.currency else None,
                "status_id": vacancy.status_id if vacancy else None,
                "status_name": vacancy.status.name if vacancy and vacancy.status else None,
                "status": {
                    "id": vacancy.status.id if vacancy and vacancy.status else None,
                    "name": vacancy.status.name if vacancy and vacancy.status else None,
                } if vacancy and vacancy.status else None,
    ...
    async def get_company_applications_page(
        ...
        try:
            stmt = (
                ...
                .where(
                    Vacancy.company_id == company_id,
                    self._visible_vacancy_status_filter(),
                )
                .options(*self._application_detail_options())
            )
            ...
            for application in applications:
                if self._is_hidden_vacancy(application.vacancy):
                    continue
    ...
    async def _get_company_user_id(
        self,
        db: AsyncSession,
        company_id: int,
    ) -> int | None:
    ...
    async def _get_application_chat_id(
        self,
        db: AsyncSession,
        application_id: int,
    ) -> int | None:
        result = await db.execute(
            select(Chat.id).where(Chat.application_id == application_id)
        )

        return result.scalar_one_or_none()

    def _get_status_chat_message(
        self,
        status_value: str,
        ...
        applicant_name = self._get_applicant_full_name(applicant)
        vacancy_title = application.vacancy.title if application.vacancy else "вакансию"

        if normalized_status == ApplicationStatus.ACCEPTED.value:
            text = (
                getattr(status_data, "interview_message", None)
                or getattr(status_data, "chat_message", None)
                or ""
            ).strip()

            if text:
                return f"Собеседование\n\n{text}"

            return (
                "Собеседование\n\n"
                f"Здравствуйте, {applicant_name}!\n\n"
                f'Благодарим Вас за отклик на вакансию "{vacancy_title}". '
                "Ваше резюме показалось нам интересным. "
                "Хотим пригласить Вас на следующий этап."
            )

        if normalized_status == ApplicationStatus.REJECTED.value:
            text = (
                getattr(status_data, "rejection_message", None)
                or getattr(status_data, "chat_message", None)
                or ""
            ).strip()
    ...
    async def _create_status_message_in_chat(
        self,
        db: AsyncSession,
        application: Application,
        company_id: int,
        ...
        status_value = self._status_value(status_data.status)

        text = self._get_status_chat_message(
        ...
        chat_id = await self._get_application_chat_id(
            db=db,
            application_id=application.id,
        )
        sender_id = await self._get_company_user_id(
            db=db,
            company_id=company_id,
        )
        ...
        try:
            ...
            if self._is_hidden_vacancy(application.vacancy):
                raise HTTPException(
                    ...
                    detail="Нельзя менять отклики по архивной или удалённой вакансии",
                )

            old_status = str(application.status or "").lower().strip()
            new_status = self._status_value(status_data.status).lower().strip()

            application.status = new_status
            application.updated_at = datetime.utcnow()

            should_create_chat_message = (
                new_status in {
            ...
            if should_create_chat_message:
                await self._create_status_message_in_chat(
                    db=db,
                    application=application,
                    company_id=company_id,
                    status_data=status_data,
    ...
    def _resume_deleted_flag(self, resume: Resume) -> bool:
        status_name = str(getattr(resume, "status_name", "") or getattr(getattr(resume, "status", None), "name", "") or "").lower()

        return bool(
            getattr(resume, "is_deleted", False)
            or getattr(resume, "deleted_at", None)
            or getattr(resume, "is_active", True) is False
            or "удален" in status_name
            or "удалено" in status_name
            or "deleted" in status_name
        )

    def _applicant_city_to_dict(self, city: City | None) -> dict | None:
        if not city:
            return None
    ...
    def map_candidate_resume(
        ...
        applicant_city = applicant.city if applicant and getattr(applicant, "city", None) else None
        applicant_city_dict = self._applicant_city_to_dict(applicant_city)
        resume_is_deleted = self._resume_deleted_flag(resume)

        applicant_photo = getattr(applicant, "photo", None) if applicant else None
        normalized_applicant_email = (
            applicant_email
            or self._get_applicant_email_from_model(applicant)
        )
        ...
        return {
            ...
            "is_active": getattr(resume, "is_active", None),
            "is_deleted": resume_is_deleted,
            "deleted_at": getattr(resume, "deleted_at", None),
            "status": getattr(resume, "status", None),
            "status_name": getattr(resume, "status_name", None),
            ...
            "applicant_city": applicant_city_dict,
            "city": applicant_city_dict,
            "applicant_city_name": applicant_city.name if applicant_city else None,
            "applicant_city_full_name": self._format_city_full_name(applicant_city) if applicant_city else None,
            "applicant_region_name": (
                applicant_city.district.region.name
                if applicant_city and applicant_city.district and applicant_city.district.region
                else None
            ),
            "applicant_district_name": (
                applicant_city.district.name
                if applicant_city and applicant_city.district
                else None
            ),
            "applicant_settlement_type_name": (
                applicant_city.settlement_type.name
                if applicant_city and applicant_city.settlement_type
                else None
            ),
            ...
            "applicant_email": normalized_applicant_email,
            "applicant_user_email": normalized_applicant_email,
            "email": normalized_applicant_email,
    ...
    async def get_candidate_resumes(
        ...
        try:
            ...
            applicant_email_map = await self._get_applicant_email_map(
                db=db,
                applicant_ids=[resume.applicant_id for resume in resumes],
            )

            mapped_items = [
                self.map_candidate_resume(
                    resume=resume,
                    applicant_email=applicant_email_map.get(resume.applicant_id),
                )
    ...
    async def get_candidate_resume_detail(
        ...
        try:
            ...
            applicant_email_map = await self._get_applicant_email_map(
                db=db,
                applicant_ids=[resume.applicant_id],
            )

            return self.map_candidate_resume(
                resume=resume,
                applicant_email=applicant_email_map.get(resume.applicant_id),
            )
